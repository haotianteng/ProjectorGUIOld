...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% PlayList is instored in the handles.playList
...
% handles    structure with handles and user data (see GUIDATA)
% Export the PlayList
[Filename,Filepath,FilterIndex]=uiputfile('*.mat','Save as','PlayList1.mat');
if(Filename==0)
    return;
end
...
switch handles.entity 
    case 'Bar'
        set(handles.text29,'String','Bar')
    case 'Spot'
end
guidata(hObject,handles);
...
% vbl = Screen('Flip', handles.win1);
% Loop: Cycle through 300 images:
Period=1/refresh/handles.frames; %Period here is a ratio of real refresh rate and the demanding refresh rate
if Period-floor(Period)>0.01
    display('Warning!The fresh frequency is not matching the frames or the multipe of frames.')
end
Period=round(Period);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.physicalHeight =  str2num(hObject.String);
guidata(hObject,handles);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.physicalWidth = str2num(get(hObject,'String'));
